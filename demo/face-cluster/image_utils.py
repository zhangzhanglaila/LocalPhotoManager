...
def render_round_pixmap(image_path: Path, size: int = 112):
    from PySide6.QtGui import QImage, QPixmap

    image = load_image_rgb(image_path)
    circular = create_circular_thumbnail(image, size=size)
    data = circular.tobytes("raw", "RGBA")
    qimage = QImage(data, size, size, size * 4, QImage.Format.Format_RGBA8888).copy()
