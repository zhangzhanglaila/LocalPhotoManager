...
class MainCoordinator(QObject):
    ...
    def _init_chat_panel(self) -> None:
        ...
        from iPhoto.gui.ui.widgets.chat_panel import ChatPanel

        if hasattr(self, "_chat_panel"):
            return

        self._chat_panel = ChatPanel(self._window)
        self._chat_panel.setVisible(False)

        # Connect signals
        self._chat_panel.message_sent.connect(self._handle_chat_message)
    ...
    def _handle_chat_message(self, message: str) -> None:
        ...
        if not self._context.settings.get("agent.semantic_search_enabled", False):
            self._chat_panel.add_message(
                tr("chat.not_available", default="Please enable semantic search first."),
                is_user=False,
            )
            return
        ...
        from PySide6.QtCore import QRunnable, Slot
        from iPhoto.agent.services.intent_parser import IntentParser, IntentType
        from iPhoto.agent.services.qa_service import QAService

        class ChatWorker(QRunnable):
            def __init__(self, intent_parser, qa_service, msg, callback):
                super().__init__()
                self.setAutoDelete(True)
            ...
            def run(self):
                try:
                    # Parse intent
                    intent = self._intent_parser.parse(self._message)

                    # Process based on intent
                    if intent.intent_type == IntentType.SEARCH:
                        self._callback("search", intent.search_terms, intent.filters)
                    elif intent.intent_type == IntentType.QUESTION:
                        result = self._qa_service.answer_question(self._message)
                        self._callback("answer", result.answer, None)
                    elif intent.intent_type == IntentType.ORGANIZE:
                        self._callback("organize", intent.query, None)
                    elif intent.intent_type == IntentType.ACTION:
                        self._callback("action", intent.action, None)
                    else:
                        self._callback("answer", intent.response or "I'm not sure what you mean.", None)
                except Exception as e:
                    logging.getLogger(__name__).error("Chat processing failed: %s", e)
                    self._callback("error", str(e), None)

        # Create services
        llm_service = library_session.get_llm_service() if hasattr(library_session, 'get_llm_service') else None
        embedding_repo = library_session.get_embedding_repository()

        intent_parser = IntentParser(llm_service=llm_service)
        ...
        def on_complete(action_type, data, extra):
            from PySide6.QtCore import QTimer
            QTimer.singleShot(0, lambda: self._process_chat_response(action_type, data, extra))

        worker = ChatWorker(intent_parser, qa_service, message, on_complete)
        QThreadPool.globalInstance().start(worker)

        # Show typing indicator
        self._chat_panel.add_typing_indicator()

    def _process_chat_response(self, action_type: str, data, extra) -> None:
        """Process chat response from background worker.

        Parameters
        ----------
        action_type : str
            Type of response: 'search', 'answer', 'organize', 'action', 'error'.
        data : any
            Response data.
        extra : any
            Extra data.
        ...
        self._chat_panel.remove_last_message()
        ...
        if action_type == "search":
            ...
            self._handle_search_requested(query)
            self._chat_panel.add_message(
                tr("chat.searching", default=f"Searching for: {query}"),
                is_user=False,
            )

        elif action_type == "answer":
            self._chat_panel.add_message(str(data), is_user=False)

        elif action_type == "organize":
            self._chat_panel.add_message(
                tr("chat.organize", default="I'll help you organize your photos. Use the Smart Organize menu for specific actions."),
                is_user=False,
            )

        elif action_type == "action":
            self._chat_panel.add_message(
                tr("chat.action", default=f"I'll {data} the selected photos."),
                is_user=False,
            )

        elif action_type == "error":
            self._chat_panel.add_message(
                tr("chat.error", default=f"Sorry, I encountered an error: {data}"),
                is_user=False,
            )
